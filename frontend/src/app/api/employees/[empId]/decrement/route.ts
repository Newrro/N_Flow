...
// POST /api/employees/[empId]/decrement - Decrement employee salary
export async function POST(
  ...
  try {
    ...
    return NextResponse.json({
      success: true,
      message: 'Salary decremented successfully',
      old_salary: oldSalary,
      new_salary: newSalary
    });
  } catch (error) {
    console.error('Error decrementing salary:', error);
    return NextResponse.json(
      { error: 'Failed to decrement salary' },
      { status: 500 }
    );
  }
}
